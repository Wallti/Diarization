...
def create_speech_recognizer(language_code, websocket):
    ...
    async def send_transcription(text, speaker_id, confidence, language, status="recognized"):
    ...
    async def transcribed_cb(evt):
        """Handle final transcriptions and validate using LLM"""
        text = evt.result.text
        confidence = getattr(evt.result, "confidence", 1.0)
        speaker = evt.result.speaker_id
        detected_lang = evt.result.language  # Get detected language (check correct attribute)
        alt_lang = "en-US"  # Modify as needed

        print(f" [DEBUG] Raw Transcription: {text} (Confidence: {confidence})")

    # Validate using LLM
        validation_result = await llm_validator.validate_text(
            text=text,
        ...
        # Send valid transcription
            await send_transcription(
                validation_result["original_text"],
                speaker,
                "recognized",
                validation_result["confidence_score"]
            )
        else:
            print(f" [DEBUG] Rejected: {validation_result['reasoning']}")
